...
export function DarkCoverTemplate({
  preheader,
  eventName,
  subheader,
  coverImageUrl,
  ...
  const opacity = 0.7; // Heavy black contrast overlay
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center justify-center text-center my-auto px-4 max-w-sm mx-auto">
        <span className="text-xs uppercase tracking-widest text-white/80 font-medium" style={customTextStyle}>
          {preheader}
        </span>
        <h1 className="text-4xl font-extrabold tracking-tight text-white mt-2 drop-shadow-sm" style={customTextStyle}>
          {eventName}
        </h1>
        {subheader && (
          <p className="text-sm text-white/90 font-medium mt-3 drop-shadow-sm" style={customTextStyle}>
            {subheader}
          </p>
        )}
        
        {/* Decorative line */}
        <div className="w-12 h-0.5 my-6 rounded-full" style={{ backgroundColor: textColor || "rgba(255,255,255,0.4)" }} />
      </div>
      ...
      <div className="relative z-10 w-full max-w-sm mx-auto pb-4">
        <button
          ...
          style={{
            ...
            color: buttonTextColor || getContrastColor(buttonColor || "#FFFFFF")
          }}
        >
